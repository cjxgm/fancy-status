...
fn cpu_node(cold: Colorf32, hot: Colorf32) -> Node {
    ...
    let cpu_total = {
        let sum: f32 = cpu_usages.iter().sum();
        let len = cpu_usages.len() as f32;
        sum / len
    };
...
fn cpu_usages() -> Vec<f32> {
    vec![0.0, 0.05, 0.1, 0.15, 0.3, 0.5, 0.7, 0.9, 1.0, 0.05]
}
